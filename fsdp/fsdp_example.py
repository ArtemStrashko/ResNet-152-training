...
def train(args, model, rank, world_size, train_loader, optimizer, epoch, sampler=None):
    ...
    ):  # Iterate over training data batches and display a progress bar
        input_ids, attention_mask, labels = (
            batch  # Unpack batch into input IDs, attention masks, and labels
        )
        input_ids, attention_mask, labels = (
            input_ids.to(rank),
    ...
    )  # Sum the total loss across all distributed processes
    if rank == 0:
        print(
            "Train Epoch: {} \tLoss: {:.6f}".format(
                epoch, total_loss[0] / total_loss[1]
            )
...
def test(model, rank, world_size, test_loader):
    ...
    total_accuracy = torch.zeros(1).to(
        rank
    )  # Initialize a tensor to store the total accuracy
    ...
    with torch.no_grad():  # Disable gradient calculation for testing
        for batch in tqdm(
        ...
        ):  # Iterate over test data batches and display a progress bar
            input_ids, attention_mask, labels = (
                batch  # Unpack batch into input IDs, attention masks, and labels
            )
            input_ids, attention_mask, labels = (
            ...
            all_predictions_in_epoch.extend(
                predicted_labels.tolist()
            )  # Append predicted labels to the list
            all_true_labels_in_epoch.extend(
                labels.tolist()
            )  # Append true labels to the list

        dist.all_reduce(
            total_accuracy, op=dist.ReduceOp.SUM
        )  # Sum the total accuracy across all distributed processes

        if rank == 0:
            print(
                f"Accuracy on Test Set: {total_accuracy[0]/len(test_loader.dataset):.4f}"
            )  # Print the accuracy
...
if __name__ == "__main__":
    ...
    )  # Get and round the maximum GPU memory consumption in GB
